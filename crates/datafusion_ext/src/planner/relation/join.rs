...
impl<'a, S: AsyncContextProvider> SqlQueryPlanner<'a, S> {
    pub(crate) async fn plan_table_with_joins(
        ...
        let left = self.create_relation(t.relation, planner_context).await?;
        match t.joins.len() {
            0 => {
            ...
            _ => {
                ...
                *planner_context = origin_planner_context.clone();
                ...
                for join in joins {
                    *planner_context = origin_planner_context.clone();
                    left = self
                        .parse_relation_join(left, join, planner_context)
                        .await?;
                }
                *planner_context = origin_planner_context;
                Ok(left)
    ...
    async fn parse_relation_join(
        &mut self,
        left: LogicalPlan,
        join: Join,
        planner_context: &mut PlannerContext,
    ) -> Result<LogicalPlan> {
        let right = self.create_relation(join.relation, planner_context).await?;
        match join.join_operator {
            ...
            JoinOperator::LeftSemi(constraint) => {
                self.parse_join(left, right, constraint, JoinType::LeftSemi, planner_context)
                    .await
            }
            ...
            JoinOperator::LeftAnti(constraint) => {
                self.parse_join(left, right, constraint, JoinType::LeftAnti, planner_context)
                    .await
            }
    ...
    async fn parse_join(
        ...
        match constraint {
            JoinConstraint::On(sql_expr) => {
                ...
                // parse ON expression
                let expr = self
                    .sql_to_expr(sql_expr, &join_schema, planner_context)
                    .await?;
