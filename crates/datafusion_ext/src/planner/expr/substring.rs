...
impl<'a, S: AsyncContextProvider> SqlQueryPlanner<'a, S> {
    pub(super) async fn sql_substring_to_expr(
        &mut self,
        expr: Box<SQLExpr>,
        ...
        let args = match (substring_from, substring_for) {
            (Some(from_expr), Some(for_expr)) => {
                let arg = self
                    ...
                    .await?;
                let from_logic = self
                    .sql_expr_to_logical_expr(*from_expr, schema, planner_context)
                    .await?;
                let for_logic = self
                    .sql_expr_to_logical_expr(*for_expr, schema, planner_context)
                    .await?;
