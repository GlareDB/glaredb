...
impl<'a, S: AsyncContextProvider> SqlQueryPlanner<'a, S> {
    pub(crate) fn parse_value(&self, value: Value, param_data_types: &[DataType]) -> Result<Expr> {
        match value {
            ...
            Value::Placeholder(param) => Self::create_placeholder_expr(param, param_data_types),
    ...
    fn parse_sql_number(&self, n: &str) -> Result<Expr> {
        if let Ok(n) = n.parse::<i64>() {
            ...
            n.parse::<f64>()
                .map(lit)
                .map_err(|_| DataFusionError::from(ParserError(format!("Cannot parse {n} as f64"))))
        }
    }
    ...
    fn create_placeholder_expr(param: String, param_data_types: &[DataType]) -> Result<Expr> {
    ...
    #[async_recursion]
    pub(super) async fn sql_interval_to_expr(
        &mut self,
        value: SQLExpr,
        schema: &DFSchema,
                ...
                match (leading_field, left.as_ref(), right.as_ref()) {
                    ...
                    (None, _, _) => {
                        let left_expr = self
                            ...
                            .await?;
                        let right_expr = self
                            .sql_expr_to_logical_expr(*right, schema, planner_context)
                            .await?;
                        return Ok(Expr::BinaryExpr(BinaryExpr::new(
                            Box::new(left_expr),
                            df_op,
