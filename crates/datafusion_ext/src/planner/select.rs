...
impl<'a, S: AsyncContextProvider> SqlQueryPlanner<'a, S> {
    ...
    pub(super) async fn select_to_plan(
        ...
        let having_expr_opt = match select.having {
            Some(having_expr) => {
                let having_expr = self
                    .sql_expr_to_logical_expr(having_expr, &combined_schema, planner_context)
                    .await?;
    ...
    async fn prepare_select_exprs(
        ...
        for e in projection {
            let e = self
                .sql_select_to_rex(e, plan, empty_from, planner_context)
                .await?;
            for v in e {
                exprs.push(v);
            }
    ...
    async fn sql_select_to_rex(
        ...
        match sql {
            SelectItem::UnnamedExpr(expr) => {
                let expr = self
                    .sql_to_expr(expr, plan.schema(), planner_context)
                    .await?;
