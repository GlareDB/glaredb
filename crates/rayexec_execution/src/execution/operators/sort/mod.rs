    ...
    fn poll_execute(
        ...
        match state {
            SortPartitionState::Consume(state) => {
                ...
                state.sort_data.push_batch(&self.layout, batch)?;
    ...
    fn poll_finalize(
        ...
        match state {
            SortPartitionState::Consume(consume_state) => {
                ...
                consume_state.sort_data.sort_unsorted_blocks(&self.layout)?;
                ...
                loop {
                    let mut block = SortBlock::new(&self.layout, consume_state.output_capacity)?;
                    let count = merger.merge_round(&mut block)?;
                    block.batch.set_num_rows(count)?;
                ...
                if operator_state.remaining == 0 {
                    ...
                    *state = SortPartitionState::Produce(PartitionStateGlobalSort {
                        merger: global_merger,
                    });
            ...
            SortPartitionState::Produce(_) => Err(RayexecError::new(
                "cannot finalize partition that's producing",
            )),
            SortPartitionState::Finished => Err(RayexecError::new(
                "cannot finalize partition that's finished",
            )),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn sort_single_partition() {
        let wrapper = OperatorWrapper::new(PhysicalSort {
        ...
        let mut output = Batch::try_new([DataType::Int32, DataType::Utf8], 1024).unwrap();

        let mut input1 = Batch::try_from_arrays([
            Array::try_from_iter([4, 3, 8]).unwrap(),
        ...
        let mut input2 = Batch::try_from_arrays([
            Array::try_from_iter([6, 9, 1]).unwrap(),
        ...
        let poll = wrapper
            ...
            .unwrap();
        assert_eq!(PollExecute::HasMore, poll); // TODO: Should be exhausted

        let expected = Batch::try_from_arrays([
            Array::try_from_iter([1, 3, 4, 6, 8, 9]).unwrap(),
            Array::try_from_iter(["f", "b", "a", "d", "c", "e"]).unwrap(),
        ])
        .unwrap();
