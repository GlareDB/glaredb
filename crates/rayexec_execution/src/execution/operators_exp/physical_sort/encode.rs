...
use crate::arrays::buffer::physical_type::{PhysicalI32, PhysicalI8, PhysicalStorage, PhysicalType};
    ...
    B: BufferManager,
{
    ...
    match flat.array_buffer.physical_type() {
        PhysicalType::Int8 => {
            encode_fixedlen::<PhysicalI8, _>(flat, selection, add_offset, offsets, buf, nulls_first, desc)
        }
        PhysicalType::Int32 => {
            encode_fixedlen::<PhysicalI32, _>(flat, selection, add_offset, offsets, buf, nulls_first, desc)
        }
        _ => unimplemented!(),
    }
}
...
where
    ...
    B: BufferManager,
{
    let input = S::get_storage(&flat.array_buffer)?;
