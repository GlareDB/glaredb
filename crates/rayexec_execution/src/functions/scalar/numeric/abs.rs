...
impl PlannedScalarFunction for AbsImpl {
    ...
    fn execute(&self, inputs: &[&Array]) -> Result<Array> {
        ...
        match input.physical_type() {
            PhysicalType::Float16 => abs_float_execute::<PhysicalF16>(input, self.ret.clone()),
            PhysicalType::Float32 => abs_float_execute::<PhysicalF32>(input, self.ret.clone()),
            PhysicalType::Float64 => abs_float_execute::<PhysicalF64>(input, self.ret.clone()),
            other => Err(unhandled_physical_types_err(self, [other])),
        }
    }
}

fn abs_float_execute<'a, S>(input: &'a Array, ret: DataType) -> Result<Array>
where
    S: PhysicalStorage<'a>,
