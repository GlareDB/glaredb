...
impl PlannedScalarFunction for CeilImpl {
    ...
    fn execute(&self, inputs: &[&Array]) -> Result<Array> {
        ...
        match input.physical_type() {
            PhysicalType::Float16 => ceil_float_execute::<PhysicalF16>(input, self.ret.clone()),
            PhysicalType::Float32 => ceil_float_execute::<PhysicalF32>(input, self.ret.clone()),
            PhysicalType::Float64 => ceil_float_execute::<PhysicalF64>(input, self.ret.clone()),
            other => Err(unhandled_physical_types_err(self, [other])),
        }
    }
}

fn ceil_float_execute<'a, S>(input: &'a Array, ret: DataType) -> Result<Array>
where
    S: PhysicalStorage<'a>,
