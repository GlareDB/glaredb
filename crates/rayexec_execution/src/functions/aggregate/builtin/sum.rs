...
mod tests {
    ...
    fn sum_i64_single_group_two_partitions() {
        ...
        states_1
            .combine(&mut states_2, Selection::selection(&[0]), &[0])
            .unwrap();

        // Get final output.
        let mut out = Array::new(&NopBufferManager, DataType::Int64, 1).unwrap();
    ...
    fn sum_i64_two_groups_two_partitions() {
        ...
        // Get final output.
        let mut out = Array::new(&NopBufferManager, DataType::Int64, 2).unwrap();
        states_1.drain(&mut out).unwrap();
    ...
    fn sum_i64_three_groups_two_partitions_with_unseen_group() {
        ...
        states_1
            .combine(&mut states_2, Selection::selection(&[0, 1]), &[0, 2])
            .unwrap();

        // Get final output.
        let mut out = Array::new(&NopBufferManager, DataType::Int64, 3).unwrap();
        states_1.drain(&mut out).unwrap();
