...
/* Commented out, doesn't seem to actually be needed. */
/* #include <malloc.h> */
 ...
 */
DSS_HUGE
set_state(int table, long sf, long procs, long step, DSS_HUGE *extra_rows, gen_state_t *state) {
    int i;
    ...
    rowcount = state->tdefs[table].base;
    rowcount *= sf;
    *extra_rows = rowcount % procs;
    ...
    for (i = 0; i < step - 1; i++) {
        if (table == LINE) /* special case for shared seeds */
            state->tdefs[table].gen_seed(1, rowcount);
        else
            state->tdefs[table].gen_seed(0, rowcount);
        ...
        if (state->tdefs[table].child != NONE)
            state->tdefs[state->tdefs[table].child].gen_seed(0, rowcount);
    }
    if (step > procs) /* moving to the end to generate updates */
        state->tdefs[table].gen_seed(0, *extra_rows);
