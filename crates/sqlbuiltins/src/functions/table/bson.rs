...
impl TableFunc for BsonScan {
    ...
    async fn create_provider(
        ...
        bson_streaming_table(store_access, sample_size as usize, source_url).await
    }
}

pub async fn bson_streaming_table(
    store_access: GenericStoreAccess,
    schema_inference_sample_size: usize,
    ...
    let mut sample = Vec::<Document>::with_capacity(schema_inference_sample_size as usize);
    ...
    'readers: for reader in readers.iter_mut() {
        while let Some(res) = reader.next().await {
            ...
            if sample.len() >= schema_inference_sample_size {
                break 'readers;
            }
        }
        first_active += 1;
