//! Tests for parsing AST nodes required for select queries.

use insta::{assert_debug_snapshot, assert_display_snapshot};
use rayexec_error::Result;
use rayexec_parser::{
    ast::{
        AstParseable, Expr, FunctionArg, GroupByList, ObjectReference, OrderByList, Query,
        SelectItem, SelectList, SelectNode, TableLike, TableList, TableOrSubquery, WildcardExpr,
    },
    parser::Parser,
    tokens::Tokenizer,
};
...
#[test]
fn parse_query() {
    assert_debug_snapshot!(parse_ast::<Query>("SELECT * FROM customer WHERE id = 1 LIMIT 5").unwrap(), @r###"
    Query {
        with: None,
        body: Select(
            SelectNode {
...
#[test]
fn parse_expr() {
    assert_debug_snapshot!(parse_ast::<Expr>("1").unwrap(), @r###"
    Literal(
        Number(
            "1",
        ),
    ...
    assert_debug_snapshot!(parse_ast::<Expr>("1+2").unwrap(), @r###"
    BinaryExpr {
        left: Literal(
            Number(
                "1",
    ...
    assert_debug_snapshot!(parse_ast::<Expr>("1+2+3").unwrap(), @r###"
    BinaryExpr {
        left: BinaryExpr {
            left: Literal(
                Number(
