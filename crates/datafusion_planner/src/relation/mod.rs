...
use datafusion::sql::sqlparser::ast::{self, FunctionArg, TableFactor};
use std::sync::Arc;
...
impl<'a, S: AsyncContextProvider> SqlQueryPlanner<'a, S> {
    ...
    fn get_constant_function_arg(&mut self, arg: FunctionArg) -> Result<ScalarValue> {
        match arg {
            FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(expr)) => match expr {
