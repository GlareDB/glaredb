...
pub fn rest(app: &mut HttpServer) {
...
async fn add_learner(mut req: Request<Arc<ApplicationState>>) -> tide::Result {
    let (node_id, api_addr, rpc_addr): (GlareNodeId, String, String) = req.body_json().await?;
    let node = GlareNode { rpc_addr, api_addr };
...
async fn change_membership(mut req: Request<Arc<ApplicationState>>) -> tide::Result {
    let body: BTreeSet<GlareNodeId> = req.body_json().await?;
    let res = req.state().raft.change_membership(body, true, false).await;
    Ok(Response::builder(StatusCode::Ok).body(Body::from_json(&res)?).build())
}

/// Initialize a single-node cluster.
async fn init(req: Request<Arc<ApplicationState>>) -> tide::Result {
...
async fn metrics(req: Request<Arc<ApplicationState>>) -> tide::Result {
    let metrics = req.state().raft.metrics().borrow().clone();

    let res: Result<RaftMetrics<GlareNodeId, GlareNode>, Infallible> = Ok(metrics);
    Ok(Response::builder(StatusCode::Ok).body(Body::from_json(&res)?).build())
}
