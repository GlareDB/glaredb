use crate::metastore::catalog::{CatalogMutator, TempCatalog};
...
impl ExecutionPlan for DropTablesExec {
    ...
    fn execute(
        ...
        let stream = stream::once(drop_tables(mutator, self.clone(), context));
...
async fn drop_tables(
    ...
    for r in plan.tbl_references {
        if temp_objects.contains_table(&r.name) {
            temp_table_drops.push(r.name.clone());
        } else {
    ...
    for temp_table in temp_table_drops {
        temp_objects.drop_table(&temp_table);
    }
